...
We moved beyond libraries and engineered a full linear regression pipeline manually. Features were normalized, a cost function was derived, gradients were implemented, and weights were optimized iteratively. The model predicted student marks — proving that understanding the mechanics beats blindly calling .fit().

1.3 — Logistic Regression for Decision Making
...
We escalated from recognizing what is in an image to detecting where. Using YOLOv8, custom datasets were prepared, trained, and validated. We tuned epochs, batch sizes, and image resolutions, then generated bounding-box predictions. This marked the transition from academic modeling to real applied AI systems capable of deployment.
